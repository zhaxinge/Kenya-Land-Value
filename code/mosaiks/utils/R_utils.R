...
makegrid <- function(meters_per_grid,  lonmin,lonmax,latmin,latmax) {
  
  # Calculate the image width and height in degrees
  image_width_degrees <-  meters_per_grid * 1 / (m_degree) 
  image_height_degrees <-  meters_per_grid * 1 / (m_degree)
  
  # Get lonVals (longitude values) - equally spaced
  lonVals <- seq(from = lonmin, to = lonmax, by = image_width_degrees)
  
  # Get latVals (latitude values)
  latVals <- c()
  currentLat <- latmax
  i <- 1
  while (currentLat > latmin) {
    ...
    i <- i + 1
    currentLat <- currentLat - image_height_degrees * cos(currentLat * pi / 180)
  }
  
  # Return the generated grid as a list
