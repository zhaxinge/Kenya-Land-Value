...
for (city in c(#'Eldoret','Embu','Garissa','Kakamega','Kericho'
  ...
  # Load JPEG image
  img <- brick(file.path(data_dir, "raw/images-new",paste0("Geomad_",city,".jpg")))
  ...
  output_dir <- file.path(data_dir, "raw/imagery-new",city)
  ...
  for (i in 1:num_cells) { #TODO:num_cells
    ...
    tryCatch({
      ...
      output_file <- file.path(output_dir, paste0(lat, "_", lon, "_", zoom, "_", image_dimensions[2], "_", image_dimensions[1], "_",ID[i], "_",".jpg"))
      
      # Save the cropped image as a JPEG file
      writeRaster(cropped_image, filename = output_file, format = "JPEG", quality = 100, overwrite=TRUE)
    }, 
    error = function(e) {
