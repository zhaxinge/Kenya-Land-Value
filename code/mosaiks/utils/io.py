...
def get_filepaths(c, app, feattype="random", is_ACS=False):
    ...
    if is_ACS:
        ...
        c.outcomes_fpath = join(
            ...
            "applications",
            app,
            "outcomes_sampled_houseprice_dense_16_640_regular_2000.csv",
        )

    return c


def get_X_latlon(c, city, mpg):
...
def load_superres_X(c):
    ...
    with open(join(c.features_dir, c.train,c.superres["features_fname"]), "rb") as f:
        data = dill.load(f)
