...
def performance_density(
    ...
    preds = np.vstack([solve.y_to_matrix(i) for i in np.atleast_1d(best_preds.squeeze())])
    truth = np.vstack([solve.y_to_matrix(i) for i in np.atleast_1d(kfold_results["y_true_test"].squeeze())])
    ...
    for i in range(n_outcomes):
        ...
        ## add 1:1 line
        jg.ax_joint.plot(this_lims, "k-", alpha=0.75)
...
def spatial_scatter_obs_v_pred(
    ...
    **kwargs
    ...
    # flatten over fold predictions
    preds = np.vstack([solve.y_to_matrix(i) for i in np.atleast_1d(best_preds.squeeze())])
    truth = np.vstack([solve.y_to_matrix(i) for i in np.atleast_1d(Y_train_clean.squeeze())])


    # get latlons in same shuffled, cross-validated order
    #ll = latlons[
    #    np.hstack([test for train, test in kfold_results.split(latlons)])
    #]
    ll = latlons[np.hstack([test for train, test in kf.split(latlons)])]
